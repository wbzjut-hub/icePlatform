...
export const syncMenusApi = (menus: MenuItem[]) => {
    return request.post('/menus/sync', menus)
}

// === 2. API卡片相关接口 ===
export const getCardsApi = () => {
    return request.get<any, ApiCard[]>('/cards/')
}

export const syncCardsApi = (cards: ApiCard[]) => {
    return request.post('/cards/sync', cards)
}

// === 3. 待办事项相关接口 ===
// 后端返回的是一个大对象: { "2023-11-01": { todos: [], notes: [] } }
export const getAllTodosApi = () => {
    return request.get<any, Record<string, { todos: TodoItem[], notes: TodoItem[] }>>('/todos/all')
}

// 同步某一天的待办
export const syncDailyTodosApi = (date: string, data: { todos: TodoItem[], notes: TodoItem[] }) => {
    return request.post(`/todos/sync/${date}`, data)
}

// === 4. 系统数据库管理接口 ===
export const getDbPathApi = () => {
    return request.get<any, { path: string }>('/system/db-path')
}

export const moveDbApi = (path: string) => {
    return request.post('/system/move-db', { path })
}
